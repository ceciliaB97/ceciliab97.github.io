...
async function initScrollList() {
    try {
        // Fetch the data created by your GitHub Action
    ...
    nextRepos.forEach(repo => {
        ...
        card.innerHTML = `
            <div class="repo-header">
                <h3>${repo.name}</h3>
                <p style="font-size: 0.9em; color: #666;">${repo.description || 'Visit repo for details.'}</p>
            </div>
            <div class="repo-footer">
                <span class="repo-tag">${repo.language || 'Code'}</span>
                <a href="${repo.html_url}" target="_blank" style="text-decoration:none; color:blue; font-weight:bold;">
                    View on GitHub →
                </a>
            </div>
        `;
